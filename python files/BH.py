...
#LOAD DATASET
def load_data():
    train_f = pd.read_excel('features/BH_test_47_02_180_13_60_trainFM.xls', header=None)
    test_f = pd.read_excel('features/BH_test_47_02_180_13_60_testFM.xls', header=None)
...
if __name__ == "__main__":
    ...
    grid_search = GridSearchCV(XGBClassifier(eval_metric='logloss'), param_grid, scoring='roc_auc', cv=cv, n_jobs=-1)
    ...
    print("Evaluating best XGBoost model on validation set:")
    train_and_evaluate(best_model, X_train_resampled, y_train_resampled, X_val, y_val)

    #Final evaluation on test set
    X_test, y_test, _ = preprocess_data(test_f, test_labels, scaler=scaler)
    print("Evaluating on the Test Set:")
    train_and_evaluate(best_model, X_train_resampled, y_train_resampled, X_test, y_test)
    
    import gc
    plt.close('all')   # Destroy all figure windows
    gc.collect()       # Force garbage collection to clean Tkinter objects
    print("✅ Done. All plots displayed, memory cleared.")